      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/80 dark:bg-slate-950/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-amber-50 dark:bg-amber-950/80 border border-amber-200 dark:border-amber-800/80 text-amber-600 dark:text-amber-400 flex items-center justify-center shadow-xs shrink-0">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-amber-50/80 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-850/80 text-amber-900 dark:text-amber-200 space-y-2">
            <div className="flex items-start gap-2.5">
              <AlertTriangle className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
              <p className="text-xs font-semibold leading-relaxed">
                Are you sure you want to remove this student from the group? Their profile will remain in the global directory.
              </p>
            </div>
            <p className="text-[11px] text-amber-800/80 dark:text-amber-300/80 pl-6.5 leading-normal">
              The student will be removed from this specific group roster and teacher's class register, but all contact info and historical records remain intact in the Master Register for future reassignment.
            </p>
          </div>
        </div>
        ...
        <div className="px-6 py-4 bg-slate-50/80 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isProcessing}
            className="px-5 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold shadow-md shadow-amber-600/25 transition-all hover:-translate-y-0.5 active:scale-95 flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            <UserMinus className="w-4 h-4" />
            <span>{isProcessing ? 'Removing...' : 'Remove from Group'}</span>
          </button>
        </div>
      </div>
    </div>
